...
class SidikaryoPerusahaanResource extends Resource
{
   ...
   public function tarikDataDetailById($id_emakaryo)
    ...
    public function tarikData_BAKL()
    {
        try {
            // Ambil data dari API Penyedia Kerja
            $response = Http::get('https://bursakerja.jatengprov.go.id/api_v2/penyedia/index');
            ...
            if ($response->successful()) {
                ...
                // Proses setiap item
                foreach ($dataFromApi as $item) {
                    // Cari mapping kabupaten/kota
                    $bridging = null;
                    if (isset($item['kabkota_kode'])) {
                        $bridging = BridgingKabkota::where('kabkota_id', $item['kabkota_kode'])->first();
                    }
                    
                    // Cek apakah data sudah ada berdasarkan id_emakaryo
                    $existingRecord = SidikaryoPerusahaan::where('id_emakaryo', $item['id'])->first();
                    ...
                    if ($existingRecord) {
                        ...
                        //     'deskripsi' => $item['deskripsi'] ?? $existingRecord->deskripsi,
                        //     'jenis_perusahaan' => $item['jenis_perusahaan'] ?? $existingRecord->jenis_perusahaan,
                        //     'nib' => $item['nib'] ?? $existingRecord->nib,
                        //     'email' => $item['email'] ?? $existingRecord->email,
                        //     'telpon' => $item['telpon'] ?? $existingRecord->telpon,
                        //     'hp' => $item['hp'] ?? $existingRecord->hp,
                        //     'website' => $item['website'] ?? $existingRecord->website,
                        //     'provinsi' => $item['provinsi'] ?? $existingRecord->provinsi,
                        //     'kabkota_kode' => $item['kabkota_kode'] ?? $existingRecord->kabkota_kode,
                        //     'kabkota' => $item['kabkota'] ?? $existingRecord->kabkota,
                        //     'kecamatan' => $item['kecamatan'] ?? $existingRecord->kecamatan,
                        //     'alamat' => $item['alamat'] ?? $existingRecord->alamat,
                        //     'kodepos' => $item['kodepos'] ?? $existingRecord->kodepos,
                        //     'cjip_kota_id' => $bridging ? $bridging->cjip_kabkota_id : $existingRecord->cjip_kota_id,
                        //     'updated_at' => now(),
                        // ];
                        
                        // Cek apakah ada perubahan
                        if ($existingRecord->fill($updateData)->isDirty()) {
                ...
                return Notification::make()
                    ->title('Data Penyedia Kerja berhasil ditarik')
                    ->body("{$countInserted} data baru ditambahkan, {$countSkipped} data sudah ada")
                    //->body("{$countInserted} data baru ditambahkan, {$countUpdated} data diperbarui, {$countSkipped} data sudah ada")
                    ->success()
                    ->send();
            } else {
                throw new \Exception('Gagal mengambil data dari API. Status: ' . $response->status());
